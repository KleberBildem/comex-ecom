...
public class Main {
    ...
    public static void main(String[] args) throws IOException, URISyntaxException {
        Object ProcessadorDeCsv = null;
        Pedido[] pedidos = ProcessadorDeCsv.toString("pedidos.csv");
        ...
        for (int i = 0; i < pedidos.length; i++) {
            ...
            for (int j = 0; j < categoriasProcessadas.length; j++) {
                if (pedidoAtual.getData().equalsIgnoreCase(categoriasProcessadas[j])) {
                ...
                if (categoriasProcessadas[categoriasProcessadas.length - 1] != null) {
                    ...
                    for (int k = 0; k < categoriasProcessadas.length; k++) {
                        if (categoriasProcessadas[k] == null) {
                            categoriasProcessadas[k] = String.valueOf(String.valueOf(pedidoAtual.getData()));
                            break;
                        }
                    }
                }
